...
import { MagnifyingGlassIcon, ArrowPathIcon, UserGroupIcon, FunnelIcon } from "@heroicons/react/24/outline";
          ...
          <table className="w-full min-w-[720px]">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 sticky top-0 z-10">
                ...
                <th className="text-left px-4 py-3 font-semibold text-gray-700 text-sm hidden sm:table-cell">Breed</th>
                <th className="text-left px-4 py-3 font-semibold text-gray-700 text-sm">Owner</th>
                <th className="text-left px-4 py-3 font-semibold text-gray-700 text-sm hidden sm:table-cell">Last Visit</th>
            ...
            <tbody>
              {Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="border-b border-gray-200 animate-pulse">
                  ...
                  <td className="px-4 py-3 hidden sm:table-cell"><div className="h-4 w-20 bg-gray-200 rounded" /></td>
                  <td className="px-4 py-3"><div className="h-4 w-28 bg-gray-200 rounded" /></td>
                  <td className="px-4 py-3 hidden sm:table-cell"><div className="h-4 w-32 bg-gray-200 rounded" /></td>
                  <td className="px-4 py-3"><div className="h-4 w-24 bg-gray-200 rounded" /></td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {items.map(p => (
                <tr key={p.id} className="border-b border-gray-200 hover:bg-gray-50 transition">
                  <td className="px-4 py-3 text-sm font-medium truncate max-w-[140px]" title={p.name} style={{ color: PRIMARY }}>{p.name}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{p.species}</td>
                  <td className="px-4 py-3 text-sm text-gray-700 hidden sm:table-cell">{p.breed || "—"}</td>
                  <td className="px-4 py-3 text-sm text-gray-700 truncate max-w-[180px]" title={p.owner?.full_name || "—"}>{p.owner?.full_name || "—"}</td>
                  <td className="px-4 py-3 text-sm text-gray-600 hidden sm:table-cell">{meta[p.id]?.last_visit ? String(meta[p.id]?.last_visit).slice(0, 16) : "—"}</td>
                  <td className="px-4 py-3 text-sm text-right space-x-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-white ring-1 ring-gray-200 hover:bg-gray-50 text-xs"
                    >
                      Details
                    </button>
                    <Link href={{ pathname: "/veterinarian/appointments", query: { patient: p.id } }} className="px-3 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-xs inline-block">
                      Consult
                    </Link>
                  </td>
                </tr>
