...
export default function Sidebar({ open, onClose, primary, collapsed, onToggleCollapse }: Props) {
  ...
  return (
    <div className={`fixed lg:static inset-y-0 left-0 z-40 w-72 ${collapsed ? "lg:w-20" : "lg:w-72"} h-screen lg:h-auto flex flex-col overflow-hidden transform lg:transform-none transition-all duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 text-white shadow-2xl lg:shadow-none`}>
      ...
      <div className="h-16 flex items-center justify-between px-3">
        <span className={`text-lg font-semibold tracking-wide ${collapsed ? "lg:sr-only" : "block"}`}>Vet Panel</span>
        <button onClick={onToggleCollapse} className="hidden lg:inline-flex items-center justify-center w-9 h-9 rounded-xl bg-white/80 text-[#0032A0] hover:bg-white active:scale-[.98] transition text-sm">{collapsed ? "»" : "«"}</button>
      </div>
      <div className="flex-1 overflow-y-auto">
        <div className={`px-4 pt-2 pb-3 text-[11px] uppercase tracking-wide text-white/70 ${collapsed ? "lg:sr-only" : "block"}`}>Main Navigation</div>
          ...
          {nav.map(item => {
            ...
            return (
              <Link
                ...
                className={`group relative flex items-center gap-3 px-3 py-2.5 rounded-2xl transition ${collapsed ? "lg:justify-center lg:py-2" : ""} ${active ? "bg-white/15 ring-1 ring-white/20" : "hover:bg-white/10 ring-1 ring-white/0 hover:ring-white/10"}`}
              >
                {active && !collapsed && <span className="absolute left-1 top-1/2 -translate-y-1/2 h-6 w-1 rounded-full bg-white" />}
                <div className={`grid place-items-center rounded-xl ${active ? "bg-white text-[#0032A0] shadow" : "bg-white/20 text-white"} w-10 h-10 ${collapsed ? "lg:w-9 lg:h-9" : ""}`}>
                ...
                {collapsed && (
                  <span className="hidden lg:block pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-2 whitespace-nowrap rounded-lg bg-white/90 text-[#0032A0] px-2 py-1 text-xs shadow opacity-0 group-hover:opacity-100">
      ...
      <div className={`absolute bottom-3 left-3 right-3 rounded-2xl bg-white/15 ring-1 ring-white/20 p-3 flex items-center gap-3 ${collapsed ? "lg:hidden" : ""}`}>
